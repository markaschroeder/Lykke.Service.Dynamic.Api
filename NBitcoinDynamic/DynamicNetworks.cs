	...
	public class DynamicNetworks
	{
		...
		private static Network RegisterMainnet()
		{
			lock (_lock)
			{
				...
				.SetBech32(Bech32Type.WITNESS_PUBKEY_ADDRESS, Encoders.Bech32("dynamic")) //reviewed. leaving for now. does not appear to be implemented in dynamic
				.SetBech32(Bech32Type.WITNESS_SCRIPT_ADDRESS, Encoders.Bech32("dynamic")) //reviewed. leaving for now. does not appear to be implemented in dynamic
		...
		private static Network RegisterTestnet()
		{
			lock (_lock)
			{
				...
				.AddDNSSeeds(new[]
				{
				new DNSSeedData("",  ""), //needs review. see chainparams.cpp ln 287. Dash has this set to empty in their repo's chainparam's as well, but not in LykkeCity/NBitcoin.Dynamic https://github.com/LykkeCity/NBitcoin.Dynamic/blob/master/NBitcoin.Dynamic/DynamicNetworks.cs ln 157
